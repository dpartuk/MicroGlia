...
def post_process_reconstruct_boundary_subcells(step2_output_dir, original_image_path, subcell_padding=5):
    """
    POST-PROCESSING PASS (Runs AFTER Step 2 is Complete):
    Scans all extracted sub-cells in step2_output_dir.
    Identifies sub-cells that touch the image frame boundary (x<=5, y<=5, x+w>=img_w-5, y+h>=img_h-5).
    Performs Convex Hull Boundary Closure Reconstruction to restore 100% of solid internal cell body pixels!
    """
    ...
    b, g, r = cv2.split(orig_img.astype(np.float32))
    cyan_diff = (b + g) / 2.0 - r
    cyan_diff = np.clip(cyan_diff, 0, 255).astype(np.uint8)
    blur = cv2.GaussianBlur(cyan_diff, (5, 5), 1.0)
    cyan_sharpened = cv2.addWeighted(cyan_diff, 1.8, blur, -0.8, 0)
    ...
    cell_dirs = glob.glob(os.path.join(step2_output_dir, "cell_*"))

    for cell_dir in cell_dirs:
        if not os.path.isdir(cell_dir): continue
        sub_files = glob.glob(os.path.join(cell_dir, "subcell_*_extracted.jpg"))
    ...
    if reconstructed_count > 0:
        print(f"  [POST-STEP 2] Reconstructed {reconstructed_count} boundary sub-cell(s) using Convex Hull Boundary Closure.")
...
def process_all_step2(step1_dir, original_image_path, output_dir):
    ...
    After Step 2 completes, automatically runs Post-Processing Boundary Reconstruction.
    """
    ...
    # RUN POST-STEP 2 BOUNDARY RECONSTRUCTION PASS AFTER STEP 2 COMPLETES
    post_process_reconstruct_boundary_subcells(output_dir, original_image_path)
